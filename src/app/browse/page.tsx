...
import { Search, Filter, Grid, List, Heart, Eye, Zap, ChevronDown, X } from 'lucide-react'
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          >
            ...
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
              Discover thousands of AI-enhanced vehicles with intelligent search and filtering
            </p>
          </motion.div>
              ...
              <div className="flex gap-3">
                ...
                <div className="flex border border-white/20 rounded-lg overflow-hidden">
                  <button
                    ...
                    className={`p-3 transition-colors ${viewMode === 'grid' ? 'bg-cyan text-black' : 'hover:bg-white/10'}`}
                  >
                    <Grid className="h-4 w-4" />
                  </button>
                  <button
                    onClick={() => setViewMode('list')}
                    className={`p-3 transition-colors ${viewMode === 'list' ? 'bg-cyan text-black' : 'hover:bg-white/10'}`}
                  >
              ...
              >
                <div className={`glass rounded-2xl overflow-hidden hover:glow-cyan transition-all duration-300 group-hover:scale-105 ${
                  ...
                  <div className={`relative ${viewMode === 'list' ? 'w-80 flex-shrink-0' : ''}`}>
                    <div className={`overflow-hidden ${viewMode === 'list' ? 'h-48' : 'aspect-video'}`}>
                      <img
                        src={car.image}
                        alt={car.name}
                  ...
                  <div className={`p-6 ${viewMode === 'list' ? 'flex-1 flex flex-col justify-between' : ''}`}>
                    <div>
                      ...
                      <div className={`gap-3 text-sm text-muted-foreground mb-4 ${
                        viewMode === 'list' ? 'flex flex-wrap' : 'grid grid-cols-2'
                      }`}>
                      ...
                      <div className="flex flex-wrap gap-2 mb-4">
                        {car.tags.map((tag) => (
                          <Badge key={tag} variant="outline" className="border-cyan/30 text-cyan">
                            {tag}
                          </Badge>
                        ))}
                      </div>
